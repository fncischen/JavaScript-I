...
function searchForCarInfo(a_id, list) {
	var index = null;

	// for loop to conduct search, for edge case that the car id's are unordered; 
  ...
  return "Car " + list[index].id + " is a " + list[index].car_make + " " + list[index].car_model + " " + list[index].car_year + ".";
}

console.log(searchForCarInfo(33,inventory));
...
function retrieveAllCarModels(list) {
	var CarModels = [];
	for(i = 0; i < list.length; i++) {
		if (!CarModels.includes(list[i].car_model)) {
			CarModels.push(list[i].car_model);
		}
	}
...
console.log(retrieveAllCarModels(inventory));

//

function retrieveAllCarYears(list) {
	var CarYears = [];
	for(i = 0; i < list.length; i++) {
		if (!CarYears.includes(list[i].car_year)) {
...
console.log(retrieveAllCarYears(inventory));

// 
function checkNumCarsOlderThan(list,year) {
	var number = 0; 
		for(i = 0; i < list.length; i++) {
		if (list[i].car_year < year) {
			 number += 1; 
		}
	}
	return number; 
...
console.log(checkNumCarsOlderThan(inventory,2000));

//

function findBMWAndAudi(list) {
	var BMWAndAudi = [];
	for(i = 0; i < list.length; i++) {

		if (list[i].car_make == "BMW" || list[i].car_make == "Audi") {
			 BMWAndAudi.push(list[i]);
		}
...
console.log(JSON.stringify(findBMWAndAudi(inventory)));
